...
#! @Returns A boolean
#! @Description
#! This checks whether the dessin is connected, so whether the group generated by $\sigma_0$ and
#! $\sigma_1$ acts transitively on $\{1, \dots, \mathrm{deg}(D)\}$.
#! @Label
#! @BeginExampleSession
#! gap> D := Dessin((1,2,3), (2,3), 3);
#! Dessin((1,2,3), (2,3), 3)
...
#! the sense that a component of degree $d$ only uses letters in $\{1, \dots, d\}$ in the permutations.
#! @BeginExampleSession
...
#! are "normalized" in the sense that a component of degree $d$ only uses letters in $\{1, \dots, d\}$
#! the permutations.
#! @BeginExampleSession
