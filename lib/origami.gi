...
InstallGlobalFunction(TwoGeneratedSmallGroups,function(d)
  local permittedSmallGroup, m, allPermittedSmallGroups;
  permittedSmallGroup:=[];
  allPermittedSmallGroups:=[];
  m:=1;
  while m<=d do
    permittedSmallGroup:= Filtered(AllSmallGroups(m), i-> Length(MinimalGeneratingSet(i))=2);
    Append(allPermittedSmallGroups, permittedSmallGroup);
    m:=m+1;
  od;
  return allPermittedSmallGroups;
end);
 ...
 InstallGlobalFunction(QROFromOrder,function(d)
    ...
    #calculating all the permitted permitted groups of order smaller equal d
   group_list:= TwoGeneratedSmallGroups(d);
    ...
    origami_list:=[];
     ...
     G:=group_list[i];
     if
     IsAbelian(group_list[i]) then
     newOrigamis:=[
     QuasiRegularOrigami(G,Group(One(G)),MinimalGeneratingSet(G)[1], MinimalGeneratingSet(G)[2]),
         QuasiRegularOrigami(G,Group(One(G)),MinimalGeneratingSet(G)[2], MinimalGeneratingSet(G)[1])
     ];
   else
